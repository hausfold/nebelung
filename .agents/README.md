...
|---|---|---|
| `AGENTS.md` | Codex, OpenCode, Cursor, Zed, Amp, Copilot-in-editor, and anything else that speaks [agents.md](https://agents.md) | **The source of truth.** Every project rule, starting with the routing rule that keeps config changes out of this repo. |
...
| `.github/copilot-instructions.md` | GitHub Copilot coding agent + code review | A **real file**, not a symlink: Copilot reads through the GitHub API, where a symlink is just a path string. Short pointer + the few facts a drive-by reviewer needs. |
...
belong to the workshop. When nebelung grows one of its own it goes in
`.agents/skills/<name>/SKILL.md`, symlinked into `.claude/skills/<name>/` and
`.opencode/skills/`, never copied.

## Caveats

- **A cloud session can do real work here** — unlike the app repos. The palette
  generator and its tests are plain Node (`node --test`, `node
  scripts/gen-ports-doc.mjs`) and need nothing from Nix; only `nix build` /
  `nix flake check` do, which is what the bootstrap is for.
