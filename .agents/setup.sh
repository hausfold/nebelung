...
# container, CI) boot a bare Linux box with no Nix. Nebelung ships as a flake —
# the palette outputs, the rendered theme tree, `nix flake check` — so without
# Nix a session can't build a port or re-lock anything (the plain `node --test`
# palette tests do run without it). This installs Determinate Nix once, puts it
# on PATH for the rest of the session, and points Nix at the agent proxy's CA.
#
# Wired up as:
